...
const { pairｰᗒindex: hash, shiftｰpair } = coordｰutilities;
const { pipeǃ } = functionalｰutilities;
const { freezeꔛ, valuesǃ } = generalｰutilities;
...
// the ripen function.
//
// When ripen gets called, it saves the current state as elders state,
...
const state = { // why use a class? we only have a singleton object here
    elders: {}, // former state vector, replaced when ripening
    adults: {}, // current state vector, computed by ripen
            ...
            for (
                ...
                x++
                    ...
                    if ( elders [ index ] ) survive.includes ( neighborsⵌ ) ?
                        adults [ index ] = [ x, y ] :
                        stiffs [ index ] = [ x, y ];
                    else born.includes ( neighborsⵌ ) ? babies [ index ] = adults [ index ] = [ x, y ] : void 0;
                    }
            },
...
export default class eternalｰmodel { // we use a class to protect it from change
    ...
    static aliveʔ̣ ( x, y ) { return state.adults [ hash ( [ x, y ] ) ] === 1; }
