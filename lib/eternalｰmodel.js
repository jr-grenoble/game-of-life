...
const { coordｰutilities, generalｰutilities, mathｰutilities } = eternalｰconfig;
// And here is what we use from these utilities
const { ထ, evenʔ̣, div2, rem2, div3, rem3 } = mathｰutilities;
const { pairｰᗒindex, shiftｰpair } = coordｰutilities;
const { pipeǃ, freezeꔛ, valuesǃ } = generalｰutilities;
...
const state = { // why use a class? we only have a singleton object here
    former: {}, // former state vector, replaced when ripening
    modern: {}, // current state vector, computed by ripen
    babies: [], // newly born cells, only valid after a call to ripen or shake
    stiffs: [], // newly deceased cells, only valid after a call to ripen or shake

    // Initialize both state vectors to 0 (all dead)
    init () { state.former = {}; state.modern = {}; },
    ...
    ripen () {
            state.save ();
            for (
                let { former, modern, babies, stiffs } = state,
                    x = 0;
                x < width;
                x++
                    ...
                    if ( former [ index ] )
                        if ( survive.includes ( neighborsⵌ ) ) modern [ index ] = [ x, y ];
                        else stiffs.push ( [ x, y ] );
                    else if ( born.includes ( neighborsⵌ ) ) babies.push (  modern [ index ] = [ x, y ] );
                    }
            },
    ...
    shake () {
            ...
            for (
                let { former, modern, babies } = state,
                    x = 0;
                x < width;
                x++
                ) for ( let y = 0; y < height; y++ ) {
...
export default class eternalｰmodel { // we use a class to protect it from change
    ...
    static sweepǃ ( burry, nurse, raise = nurse ) {
            raise.setup ();
            valuesǃ ( state.modern ).forEach ( pair => raise ( ...pair ) );
            raise.close ();
            burry.setup ();
