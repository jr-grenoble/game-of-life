...
const { triangle } = tiles; // these are the tessalations we have to know about in this module
...
const { pipeǃ, functionʔ̣, valuesǃ } = generalｰutilities;
...
const neighborhood = { // why use a class, we only have a singleton object here
    ...
    flanks: { // functions mapping out of board coordinate pairs to legit coordinates on a mapped flank
        ...
        mapper: flank => [ bottom, left, right, top ].
            ...
            reduce ( ( accum, side ) => pipeǃ ( accum, neighborhood.flanks [ flank ] [ side ] ), neighborhood.flanks [ flank ].drop ),
    ...
    // Initialize neighborhood (allocate the cache array once we know dimensions)
    init () { neighborhood.cache.neighbors = new Array ( maxw * maxh ); },
...
const state = { // why use a class, we only have a singleton object here
    ...
    babies: null, // newly born cells, only valid after a call to ripen
    stiffs: null, // newly deceased cells, only valid after a call to ripen
    ...
    clear () { state.save (); state.modern.fill ( 0 ); },

    // Fill current state with random values
    shake () { state.save (); state.modern = Uint8ClampedArray.from ( { length: maxw * maxh }, () => Math.random () < 0.5 ? 0 : 1 ); },

    ripen () {
            // Hugely important: keep the save statement before constant declarations, as it swaps former and current states
            state.save ();

            const { former, modern, babies, stiffs } = state;
            const add = ( a, b ) => a + b;
            const countｰlifesｰaround = index => neighborhood.cache.neighbors [ index ].map ( i => former [ i ] ).reduce ( add );
            const applyｰrule = ( x, y ) => {
                    const index = pairｰᗒindex ( x, y );
                    const neighborsⵌ = countｰlifesｰaround ( index );
                    modern [ index ] =
                        former ( index ) ?
            ...
            for ( let x = 0; x < width; x++ ) for ( let y = 0; y < height; y++ ) applyｰrule ( x, y );
            }
    };
...
export default class eternalｰmodel { // we use a class to protect it from change
    ...
    static breedǃ ( x, y ) { state.modern [ pairｰᗒindex ( [ x, y ] ) ] = 1; }
    // Turn a cell dead, doesn't check whether x and y are valid.
    static chokeǃ ( x, y ) { state.modern [ pairｰᗒindex ( [ x, y ] ) ] = 0; }
    // Compute next state for the whole board,
    // and apply burry ( x, y ) to each dead cell and nurse ( x, y ) to each newborn cell, in that order.
    static cycleǃ ( burry, nurse ) {
            state.ripen ();
