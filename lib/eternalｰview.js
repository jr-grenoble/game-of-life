...
const game = new class extends board {
    ...
    move ( event ) {
            ...
            this.ⴵ.move = setTimeout ( () => {
                    this.ⴵ.move = null;
                    const coords = footer.div ( "coordinates" );
                    const { x, y } = this.pos ( event );
                    coords.innerHTML = `⌖ ${ x },${ y }<br />⌗ ${ this.width }, ${ this.height }`;
                    }, 66 );
            }
    resize ( /* event */ ) {
            ...
            this.ⴵ.resize = setTimeout ( () => {
                    ...
                    switch ( this.grid.type ) {
                        case "hexagon": w += 2 * this.grid.width / 3; break;
                        case "square": break;
                        case "triangle": if ( ᒪｘᒧ ( 2 * ᒪｘᒧ ( w ) / this.grid.width ) !== ᒪｘᒧ ( 2 * w / this.grid.width ) ) w += this.grid.width / 2; break;
...
// ==| Mouse coordinates |=====================================================
const coordinates = footer.add ( "coordinates" );
coordinates.style.fontSize = "larger";
