...
class inputｰnumberｰlist extends input {
    ...
    set value ( value ) {
            ...
            const validate = val => {
                    const { mode, range } = config.data.rules.neighbourhood;
                    const max = mode === "Moore" ?
                        ( 2 * range + 1 ) ** 2 - 1 :
                        2 * range * ( range + 1 );
...
const game = new class extends board {
    constructor () {
            ...
            propertiesꔛ ( this, {
                ...
                step: {
                    value: new class extends button {
                        ...
                        press ( /* event */ ) {
                                const ns = this.game.nextｰstate ?
                                    this.game.nextｰstate ( this.game.state ) :
                                    new state ( this.game ).randomize ();
                                this.game.redraw ( ns );
                                this.game.state = ns;
                                }
                        } ( this )
            ...
            this.nextｰstate = null; // by default, the view doesn't know how to step forward; the model provides that function
            }
    // ==| Mouse events |===========================================================
    pos ( event ) {
            const w = this.width;
    ...
    redraw ( newｰstate ) {
            ...
            // then draw new state cells in live colors
            ctx.save ();
            this.draw ( newｰstate || this.state, config.data.grid.live );
            }
    };
...
export default class eternalｰview {
    ...
    constructor () {
            ...
            game.rules.born.value = config.data.rules.born;
            game.rules.survive.value = config.data.rules.survive;
