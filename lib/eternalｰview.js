...
const defaults = {
    ...
    speed: { value: 0 },
    trail: { visible: true, color: "#bfffbf", alpha: 0.5 }, // most recent color, gradient goes to white by increasing alpha at each step
...
const neighborhoods = {
    ...
    vonｰneumann: {
        ...
        hexagon ( range, index, x, y ) { return neighborhoods [ surroundings.moore ] [ tiles.hexagon ] ( range, index, x, y ); },
        square ( range, index, x, y ) {
                /*
                 * ▫︎(x,y,i,j) allows to cover a quarter of the vicinity of ◾︎ (x,y):
                 * i →
        ...
        triangle ( range, index, x, y ) {
                // TODO !! fake function
                return neighborhoods [ surroundings.moore ] [ tiles.triangle ] ( range, index, x, y );
                }
        }
    };
...
class state {
    ...
    static get vicinity () { return config.data.rules.neighborhood.vicinity; }
    ...
    nextｰstate () {
            ...
            for (
                ...
                x++
                    ...
                    if ( ! this.liveʔ̣ ( index ) && born.include ( pop ) ) newｰstate.breed ( index );
...
const game = new class extends board {
    constructor () {
            ...
            propertiesꔛ ( this, {
                ...
                config: {
                    value: new class extends modalｰbutton {
                        constructor ( game ) {
                                ...
                                this.canvas = new canvas ( "neighborhood-canvas", "neighborhood-svg" );
                        ...
                        press ( event ) {
                                super.press ( event );
                                this.zoom.factor.value = config.data.zoom.value;
                                ...
                                this.colors.live.value = config.data.grid.live;
                                this.colors.trail.value = config.data.trail.color;
                                this.colors.alpha.value = config.data.trail.alpha;

                                this.effects.bottom.value = config.data.geometry.bottom;
                                this.effects.left.value = config.data.geometry.left;
                                this.effects.right.value = config.data.geometry.right;
                                this.effects.top.value = config.data.geometry.top;

                                this.born.display ( config.data.rules.born );
                                this.survive.display ( config.data.rules.survive );
                                this.neighborhood.vicinity.value = config.data.rules.neighborhood.vicinity;
                                this.neighborhood.range.value = config.data.rules.neighborhood.range;
                        ...
                        redraw () {
                                ...
                                const r = config.data.rules.neighborhood.range;
                                const s = 2 * r + 2; // max index of squares per row in neighborhood grid
                                const q = 100 / ( s + 1 ); // square side
                                // center cell first
                                ctx.fillStyle = config.data.grid.live;
                                ctx.fillRect ( p + q + r * q, p + q + r * q, q - p, q - p );
                                ...
                                if ( config.data.rules.neighborhood.vicinity !== surroundings.vonｰneumann ) return;
                ...
                step: {
                    value: new class extends button {
                        ...
                        press ( /* event */ ) {
                                const ns = new state ( this.game ); // build empty new state
                                // fill new state
                                if ( this.game.nextθ ) this.game.nextθ ( this.game.state, ns );
                                else ns.randomize ();
            ...
            this.nextθ = null; // by default, the view doesn't know how to step forward; the model provides that function
            }
    // ==| Mouse events |===========================================================
    pos ( event ) { // return grid coordinates from where the event occurred
            ...
            switch ( this.tiling ) {
                ...
                case tiles.triangle:
                    // we bring x,y coordinates into the square cell i,j; i.e. (x,y) ∈ ⟦0,1⟧²
                    [ x, y ] = [ x - i, y - j ];
                    // Depending on whether i and j are of the same parity, we text where x,y is with respect to the square diagonal:
    ...
    click ( event ) { // toggle state of clicked cell
            ...
            const { ctx, w, h, pw, cellθ } = this.drawｰctx ();
            ctx.save ();
            ctx.fillStyle = color;
            ctx.strokeStyle = "transparent";
            cellθ ( x, y, ctx, w, h, pw );
            ctx.restore ();
            log ( `toggled ${ x },${ y }` );
            }
    dblclick ( event ) {
    ...
    // ==| Accessors |==============================================================
    set nextｰstate ( f ) { if ( ! f || functionʔ̣ ( f ) ) this.nextθ = f; else throw TypeError ( "Invalid function!" ); }
    ...
    // ==| Drawing |================================================================
    // All of these functions should be encapsulated !
    drawｰctx () {
            //
            const { ctx } = this.canvas;
            const { height, scale, width } = this.grid;
            let w = width, h = height, pw = 1 / scale, cellθ = this.drawｰsqr;
            if ( this.tiling !== tiles.square ) {
                pw /= 2;
                switch ( this.tiling ) {
                    case tiles.hexagon:
                        w /= 3;
    ...
    drawｰsqr ( x, y, ctx, w, h, pw ) {
            ctx.fillRect ( x * w + pw, y * h + pw, w - pw, h - pw );
            }
    drawｰtri ( x, y, ctx, w, h, pw ) {
            ctx.beginPath ();
            let px = 3 * x * w / 2 + pw;
            let py = ( 2 * y + x % 2 + 1 ) * h + pw;
            if ( ( x + y ) % 2 === 0 ) {
    ...
    redraw ( newｰstate ) {
            ...
            if ( newｰstate && config.data.trail.visible ) {
                ...
                this.draw ( this.state, config.data.trail.color );
                }
            ...
            this.draw ( this.state, config.data.grid.live );
            }
    };
...
export default class eternalｰview {
     ...
     */
    constructor () {
            // only set values of visible control knobs
            ...
            game.config.colors.trail.value = config.data.trail.color;
