...
const defaults = {
    grid: { visible: true, color: "#e0e0e0", type: "square", baseｰsize: 10, stepｰsize: 1, step: 1 },
    speed: { value: 0 },
                        ...
                        constructor () {
                                ...
                                tiling.forEach (
                                    option => {
                                            ...
                                            const url = style.getPropertyValue ( `--${ option }` );
                                            this.gridｰtypes [ name ] = new class extends radioｰbutton {
                                                constructor () {
                                                        ...
                                                        // Possibly move type to grid; if not possible explain why !!
                                                        this.baseｰheight = parseInt ( url.replace ( /[^-]height\s*=\s*'([0-9.]*)/, "$1" ), 10 );
                                                        this.baseｰwidth = parseInt ( url.replace ( /[^-]width\s*=\s*'([0-9.]*)/, "$1" ), 10 );
                                                        }
