...
class input extends elements {
    constructor ( name, configｰdata = config.data, configｰname = name ) {
            super ( name );
    ...
    set value ( value ) {
            ...
            // we must set config.data….value first, before calling virtual select !
            this.elem.value = this.configｰdata [ this.configｰname ] = value;
            config.store ();
            this.select ( value );
            }
    select ( value ) { virtualⵢ ( value ); }
    }

class modalｰbutton extends button {
    constructor ( name ) {
            super ( name );
            ...
            propertiesꔛ ( this, {
                        ...
                        constructor ( game ) {
                                ...
                                this.zoom = {
                                    mode: new class extends input {
                                        ...
                                        select () { game.zoom.slide (); }
                                        },
                                    step: new class extends input {
                                        constructor () { super ( "scaling-step", config.data.grid.scaling, "step" ); }
                                        select () { game.zoom.slide (); }
                                        },
                                    shift: new class extends input {
                                ...
                                this.colors = {
                                    grid: new class extends input {
                                        ...
                                        select () { if ( game.grid.visible ) game.grid.show (); }
                                        },
                                    trail: new class extends input {
                                        constructor () { super ( "trail-color", config.data.trail, "color" ); }
                                        select () { if ( game.trail.visible ) game.trail.show (); }
                                        },
                                    };
                                }
                        press ( event ) {
                                ...
                                tiling.forEach (
                                    option => {
                                            ...
                                            this.tilingｰbuttons [ name ] = new class extends radioｰbutton {
                                                ...
                                                select () { this.grid.tiling = name; } // Uh ho … danger !
                                                } ( this );
                ...
                speed: {
                    value: new class extends slider {
                        ...
                        slide () { log ( `sliding speed to ${ config.data.speed.value }` ); }
                        }
                    },
                // ==| Zoom |==================================================================
                zoom: {
