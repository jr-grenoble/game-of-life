...
const game = new class extends elements {
    ...
    get height () { return ᒪｘᒧ ( this.canvas.height / this.grid.height ); }
    get width () { return ᒪｘᒧ ( this.canvas.width / this.grid.width ); }
    ...
    color ( x, y, color ) {
            ...
            switch ( this.type ) {
                case "hexagon": return this.hexagonｰcolor ( ctx, x, y, color, width, height, scale );
                case "square": return this.squareｰcolor ( ctx, x, y, color, width, height, scale );
                case "triangle": return this.triangleｰcolor ( ctx, x, y, color, width, height, scale );
                default: throw TypeError ( `Invalid game type: ${ this.type }` );
                }
            }
    squareｰcolor ( ctx, x, y, color, width, height, scale ) {
            // compute where top-left corner is and draw from there
            const penｰwidth = 1 / scale;
            ctx.fillStyle = color;
            ctx.fillRect ( x * width + penｰwidth, y * height + penｰwidth, width - penｰwidth, height - penｰwidth );
            }
    // EXPERIMENTAL ZONE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    // ==| Zoom |==================================================================
    ...
    resize () {
            ...
            setTimeout ( () => { // defer real resizing/redrawing of the canvas after the CSS flex box has redrawn boundaries
                    // if we have a hex grid, we must remove 1/3 of the scaled width from the result
                    canvas.width = this.grid.type === "hexagon" ? w - this.grid.width / 3 : w;
                    canvas.height = h;
                    this.redraw ();
                    }, 0 );
            }
