...
const defaults = {
    ...
    grid: { visible: true, color: "#e0e0e0", live: "#000000", tiling: square, scaling: { mode: "linear", step: .1 } },
    ...
    get nextｰstate () {
            ...
            if ( state.cached ) try {
                for ( let x = 0; x < width; x++ ) for ( let y = 0; y < height; y++ ) {
                    ...
                    const pop = neighborsⵌ ( state.cache [ index ] );
            ...
            const applyｰrules = ( x, y ) => coordinates => {
                    ...
                    const pop = neighborsⵌ ( coordinates );
                    if ( this.liveʔ̣ ( index ) ) newｰstate [ survive.includes ( pop ) ? "breed" : "kill" ] ( index );
                    else if ( born.includes ( pop ) ) newｰstate.breed ( index );
                    };
...
class canvas extends elements { // canvas elements do not handle any events
    ...
    static drawｰtri ( x, y, ctx, w, h, pw ) {
            ...
            if ( ( x + y ) % 2 === 0 ) {
                ...
                ctx.lineTo ( px, py += h - 2 * pw );
                ctx.lineTo ( px += w - 2 * pw, py -= h / 2 - pw );
                }
            else {
    ...
    drawｰctx () {
            const { color, height, scale, tiling, visible, width } = this.game.grid;
            const { ctx } = this;
            let w = width, h = height, pw = visible ? 1 / scale : 0, cellθ = ( x, y ) => canvas.drawｰsqr ( x, y, ctx, w, h, pw );
            if ( tiling !== square ) {
                pw /= 2;
                switch ( tiling ) {
                    case hexagon:
            ...
            return { ctx, w, h, pw, cellθ, stroke: color, visible };
            }
    drawｰcell ( x, y, color ) {
                                    ...
                                    const step = timestamp => {
                                            ...
                                            if ( ticks++ % speed === 0 ) {
                                                this.game.comments.innerHTML = `
                                                    ${ ( elapsed / 1000 ).toPrecision ( 3 ) } s elapsed (${ steps++ } steps)<br />
                                                    ${ ᒪｘᒧ ( elapsed / steps ) } ms per step`;
