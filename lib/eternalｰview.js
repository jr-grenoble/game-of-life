// TODO: reflect change in range or tiling or neighborhood in rules (max)
// TODO: move computedStyle out of resize
// TODO: get rid of game class crap, use regular objects
...
const { functionalｰutilities, generalｰutilities, mathｰutilities } = eternalｰconfig;
...
const { ᒪｘᒧ, ᒥｘᒧ, evenʔ̣, div2 } = mathｰutilities;
...
// User modifiable configuration parameters. These are globally accessible copies
// of configuration parameters, allowing module code to access these without
// prefixing them. Whenever they change, they are stored in the configuration
// store.
...
class canvas extends element { // canvas elements do not handle any events
    ...
    drawｰctx () {
            const { height, width } = this.game.grid; // scaled width and height of cells // TODO: use globals
...
class inputｰrule extends input { // inputｰrule elements allow for entering lists and ranges of numbers corresponding to rules
    ...
    set value ( value ) {
            ...
            const prevｰvalue = inputｰrule.validate ( inputｰrule.string ( this.value ) );

            // OK, if the browser thinks it's legit, check validity further
            if ( this.elem.checkValidity () ) try {
                super.value = inputｰrule.validate ( value );
                }
                catch ( e ) {
    ...
    change () { config.store (); }
    display ( value ) { this.elem.value = inputｰrule.string ( value ); }
            ...
            propertiesꔛ ( this, {
                ...
                config: {
                    value: new class extends modalｰbutton {
                        constructor ( game ) {
                                ...
                                this.effects = { // REPLACE extended classes by input instances
                                ...
                                this.rules = {
                                    born: new class extends inputｰrule {
                                        constructor ( game ) { super ( "born", config.data.rules, "born" ); this.game = game; }
                                        } ( this.game ),
                                    mutate: void 0,
                                    survive: new class extends inputｰrule {
                ...
                trail: {
                    value: new class extends toggle {
                        ...
                        hide () { this.game.redraw ( false ); }
                        show () { this.game.redraw ( false ); }
                        } ( this )
                    },
                // ==| Watch position |========================================================
                watch: {
                    value: new class extends element {
                        constructor ( game ) {
