...
const game = new class extends board {
    ...
    move ( event ) {
            ...
            this.ⴵ.move = window.requestAnimationFrame ( () => {
                    ...
                    const [ x, y ] = this.pos ( event );
                    this.coordinates.innerHTML = `<i class="material-icons">my_location</i> <sup>${ x },${ y }</sup>`;
                    this.coordinates.innerHTML += `<br /><i class="material-icons">crop_free</i> <sup>${ this.width }, ${ this.height }</sup>`;
                    } );
            }
    resize ( /* event */ ) {
