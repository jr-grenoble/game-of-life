...
const game = new class extends elements {
                ...
                grid: {
                    value: new class extends toggle {
                                    ...
                                    option => {
                                            ...
                                            this.types [ name ] = new class extends radioｰbutton {
                                                constructor () {
                                                        ...
                                                        //
                                                        // RESUME HERE
                                                        // DEFINE FUNCTION BELOW TO GET ACTUAL HEIGHT AND WIDTH
                                                        // CAREFULLY ASSESS HOW SCALE IS computed
                                                        // Possibly move type to grid; if not possible explain why !! Trailing blank intended
                                                        this.baseｰvisibleｰurl = style.getPropertyValue ( `--${ name }` );
                                                        this.baseｰinvisibleｰurl = this.baseｰvisibleｰurl.replace ( /stroke\s*=\s*'[^']*'/, "stroke='transparent'" );
                                                        this.baseｰheight = parseFloat ( this.baseｰvisibleｰurl.replace ( /.*[^-]height\s*=\s*'([0-9.]*).*/, "$1" ) );
                                                        this.baseｰwidth = parseFloat ( this.baseｰvisibleｰurl.replace ( /.*[^-]width\s*=\s*'([0-9.]*).*/, "$1" ) );
                                                        }
                                                select () { game.type = name; } // Uh ho … danger !
                                                };
                        ...
                        get scale () {
                                ...
                                const zm = config.data.zoom.value;
                                switch ( sc.mode ) {
                        ...
                        // CURRENTLY THE type accessors are mute, in fact we use game.type instead of this. type. Trailing blank intended
                        ...
                        zoom () {
                                log ( `computed scale is thus ${ this.scale }` );
                                keysǃ ( this.types ).forEach (
                                    type => {
                                            const style = getComputedStyle ( document.body );
                                            let url = style.getPropertyValue ( `--${ type }` );
                                            url = url.replace ( /stroke-width\s*=\s*'[^']*'/, `stroke-width='${ 1 / this.scale }'` );
                                            document.documentElement.style.setProperty ( `--${ type }`, url );
    ...
    resize () {
            ...
            const w = ᒪｘꓹｓᒧ ( ᒪｘᒧ ( cs.width ) - ᒪｘᒧ ( cs.paddingLeft ) - ᒪｘᒧ ( cs.paddingRight ) - this.grid.types [ this.type ].baseｰwidth / this.grid.width, this.grid.width ) + 1;
            const h = ᒪｘꓹｓᒧ ( ᒪｘᒧ ( cs.height ) - ᒪｘᒧ ( cs.paddingTop ) - ᒪｘᒧ ( cs.paddingBottom ) - this.grid.types [ this.type ].baseｰheight / this.grid.height, this.grid.height ) + 1;
