...
const gridｰtype = freezeꔛ ( { triangle: "triangle", square: "square", hexagon: "hexagon" } );
...
const defaults = {
    grid: { visible: true, color: "#e0e0e0", type: gridｰtype.square, baseｰsize: 8, stepｰsize: 2 },
    speed: { value: 0 },
            ...
            propertiesꔛ ( this, {
                ...
                zoom: {
                    value: new class extends slider {
                        ...
                        slide ( value ) {
                                super.slide ( value ); log ( `sliding zoom to ${ value }: cell size is ${ game.grid.size }px, scale is ${ value }` );
                                game.grid.scale ();
                                game.resize ();
                                // WE MUST REDRAW HERE !
