...
function makeｰmovable ( movingｰelem, ...targetｰelems ) {
        let dx = 0, dy = 0;
        movingｰelem.draggable = true;

        const { offsetLeft, offsetTop, offsetWidth, offsetHeight } = movingｰelem;
        movingｰelem.style.margin = "0";
        movingｰelem.style.right = "";
        movingｰelem.style.bottom = "";
        movingｰelem.style.left = `${ offsetLeft }px`;
        movingｰelem.style.top = `${ offsetTop }px`;
        movingｰelem.style.width = `${ offsetWidth }px`;
        movingｰelem.style.height = `${ offsetHeight }px`;

        movingｰelem.addEventListener ( "dragstart", e => {
                e.dataTransfer.setData ( "text/plain", movingｰelem.id );
                const { offsetLeft, offsetTop } = movingｰelem;
                dx = offsetLeft - e.x;
                dy = offsetTop - e.y;
                } );
        targetｰelems.forEach ( targetｰelem => {
                targetｰelem.addEventListener ( "dragover", e => { e.preventDefault (); } );
                targetｰelem.addEventListener ( "drop", e => {
                        e.preventDefault ();
                        if ( e.dataTransfer.getData ( "text/plain" ) !== movingｰelem.id ) return; // not for us !
                        movingｰelem.style.left = `${ e.x + dx }px`;
                        movingｰelem.style.top = `${ e.y + dy }px`;
                        } );
                } );
        }
...
class modalｰbutton extends button { // modal buttons prepare to display a modal popup on top of a frosted window; everything remains hidden by default
    constructor ( name ) {
            ...
            makeｰmovable ( this.popup, this.frosted );
